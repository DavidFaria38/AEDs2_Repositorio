...
class TP02Q18{
	...
	static void quicksort_recur(Jogador []player, int esq, int dir, int KEY_STOP, RecordAlgoritmo record){
		int i = esq, j = dir;
		String pivoEstado = player[(esq+dir)/2].getEstadoNascimento();
		String pivoNome = player[(esq+dir)/2].getNome();

		while(i <= j){
			while((player[i].getEstadoNascimento().compareTo(pivoEstado) < 0) || 
				  ((player[i].getEstadoNascimento().compareTo(pivoEstado) == 0) && 
				  (player[i].getNome().compareTo(pivoNome) < 0)) ){ 
				record.sumCmp(3);
				i++;

			} // end while
			while((player[j].getEstadoNascimento().compareTo(pivoEstado) > 0) || 
				  ((player[j].getEstadoNascimento().compareTo(pivoEstado) == 0) && 
				  (player[j].getNome().compareTo(pivoNome) > 0)) ){
				record.sumCmp(3);
				j--;
			} // end while
		...
		if(esq < j)
			quicksort_recur(player, esq, j, KEY_STOP, record);
		if(i < KEY_STOP && i < dir)
			quicksort_recur(player, i, dir, KEY_STOP, record);
	} // end quicksort_recur()

	/**
	 * - Metodo que ordena(Insercao) o array de objetos, tendo como key o Estado de nascimento.
	 * @param player - Array de objetos.
	 * @param n - Tamanho do array.
	 * @param record - Armazenas os status do metodo.
	 */
	static void quicksort_Parcial(Jogador []player, int n, final int KEY_STOP, RecordAlgoritmo record){
		quicksort_recur(player, 0, n-1, KEY_STOP, record);
	} // end quicksort_Parcial()

	public static void main(String []arg){
		final int KEY_STOP = 10;
		int nPlayer = 0;	
		...
		quicksort_Parcial(player, nPlayer, KEY_STOP, record);	
		record.endCurrentTime();
	
		printPlayers(player, KEY_STOP);	

		record.recordTime("699415_quicksort.txt");
	} // end main()
}
