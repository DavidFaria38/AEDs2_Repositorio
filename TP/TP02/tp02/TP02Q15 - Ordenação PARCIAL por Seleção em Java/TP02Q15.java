...
class TP02Q15{
	 ...
	 */
	static void selecao_Parcial(Jogador []player, int n, final int KEY_STOP, RecordAlgoritmo record){
		int i, j, menor;
		Jogador temp = new Jogador();
		
		for(i = 0; i < KEY_STOP; i++){
			menor = i;
			for(j = (i+1); j < n; j++){
				record.sumCmp(2);
	...
	public static void main(String []arg){
		...
		selecao_Parcial(player, nPlayer, KEY_STOP, record);	
		record.endCurrentTime();
	
		printPlayers(player, 10);	
