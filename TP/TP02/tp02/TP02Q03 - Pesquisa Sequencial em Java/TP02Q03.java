...
class TP02Q03{
	...
	/**
	 * - Metodo verifica se existe um elemento name dentro de array de obejtos.
	 * @param joagador - array objeto
	 * @param nJogador - quantidade de Obejtos dentro do array
	 * @param name - nome a ser verificado
	 * @return true ou false, caso array de objetos contanha elemento name
	 */
	static boolean isJogador(Jogador[] jogador, int nJogador, String name, int[] comp){
		int i;
		boolean result = false;
		for(i = 0; i < nJogador; i++){
			if(jogador[i].getNome().contains(name)){
				result = true;
				nJogador = i;
			} // end if
		} // end for
		comp[0] += i;
		//System.out.println("Comp: " + comp[0]);
		return result;
	} // end isJogador()

	/**
	 * - Metodo cria um arquivo txt com o tempo de execução e quantidade de comparações feita pelo algoritmo.
	 * @param time - tempo de execucao do programa (milissegundos)
	 * @param comp - comparações feitas pelo algoritmo
	 */
	static void logStatus(double time, int[] comp){
		Arq.openWrite("./699415_sequencial.txt");
		Arq.println("699415\t" + time/1000 + "\t" + comp[0]);
		Arq.close();
	} // end logStatus()
	...
	public static void main(String []arg){
		...
		double start, end;
		int[] comp = new int[1];
		String input, nameInput, fileName = "/tmp/players.csv";
		Jogador[] jogador = new Jogador[100];

		start = System.currentTimeMillis(); // log time

		input = MyIO.readLine();

		// armazenamento do array de objetos
		while(!isFIM(input)){
			idInput = Integer.parseInt(input);
			
			jogador[count] = new Jogador();
			jogador[count].ler(idInput, fileName);
			
			input = MyIO.readLine();
			count++;
		} // end while
		
		// input e analize de dados
		nameInput = MyIO.readLine();
		while(!isFIM(nameInput)){
			if(isJogador(jogador, count, nameInput, comp))
		...
		end = System.currentTimeMillis(); // log time
		logStatus(end - start, comp); // save log
	} // end main()
}
