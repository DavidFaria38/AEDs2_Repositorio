...
/*
	Erro, primeiro scanf esta lendo nada, com isso add mais 1 no arrayName[]
		arrayName[0] esta vazio.
*/
	getchar(); // limpa a entrada
	scanf("%[^\n]", nameInput);

	while(!isFIM(nameInput)){
		for(i = 0; i < strlen(nameInput); i++){
			if(nameInput[i] != '\r' && nameInput[i] != '\n') // remove \n e \r da entrada
				arrayName[n][i] = nameInput[i];
			else
				i = strlen(nameInput);
		} // end for
		//printf("arrayName[%i]: %s\n",n ,arrayName[n]);
		n++;
