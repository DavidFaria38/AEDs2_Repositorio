...
/**
 * - Função realiza ordenação inserção PARCIAL de players tendo como chave primaria o ano de nascimento do jogador.
 ...
 */
void insertion_PARCIAL(Jogador **player, int n, StatusOrd *status){
	int i, j;
	Jogador *tmp = newJogador();
	for(i = 1; i < n; i++){
		clone(tmp, player[i]);
		status->mov += 3;
		j = (i < KEY_STOP) ? i-1 : KEY_STOP-1;

		while((j >= 0) && (player[j]->anoNascimento > tmp->anoNascimento)){
			swap(player, j+1, j);
			status->comp += 1;
			status->mov += 3;
			j--;
		...
		for(j = 0; j < KEY_STOP - 1; j++){
			if( (player[j+1]->anoNascimento == player[j]->anoNascimento) && 
				(strcmp(player[j+1]->nome, player[j]->nome) < 0)){
					swap(player, j+1, j);
					status->comp += 2;
					status->mov += 3;
			} // end if
...
int main(void){
	...
	startClock(status);
	insertion_PARCIAL(player, nPlayer, status);
	endClock(status);

	for(int i = 0; i < KEY_STOP; i++)
		imprimir(player[i]);

	recordTime(status, strdup("699415_insercao.txt"));
	return 0;
} // end main()
