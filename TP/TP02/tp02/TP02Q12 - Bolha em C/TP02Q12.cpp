...
/**
 * - Função realiza ordenação BOLHA de players tendo como chave primaria o ano de nascimento do jogador.
 ...
 */
void bolhasort(Jogador **player, int n, StatusOrd *status){
	int i, j, k;
	for(i = (n-1); i > 0; i--){
		for(j = 0; j < i; j++){
			if(player[j]->anoNascimento > player[j + 1]->anoNascimento){
				swap(player, j, j+1);
				status->comp+=1;
				status->mov+=3;
			} 
			else if((player[j]->anoNascimento == player[j + 1]->anoNascimento) && 
					(strcmp(player[j]->nome, player[j+1]->nome) > 0) ){
				swap(player, j, j+1);
				status->comp+=2;
				status->mov+=3;
			} // end else if
...
int main(void){
	...
	startClock(status);
	bolhasort(player, nPlayer, status);
	endClock(status);

	for(int i = 0; i < nPlayer; i++)
		imprimir(player[i]);

	recordTime(status, strdup("699415_bolha.txt"));
	return 0;
} // end main()
