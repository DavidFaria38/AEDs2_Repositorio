...
/* ================= Metodos set ================= */
	public void sumComp(int value){this.comp += value;}
...
class TP02Q05{
	...
	/**
	 * - Metodo que ordena o array de objetos, tendo sua key o nome.
	 * @param player - Array de objetos.
	 * @param n - Tamanho do array.
	 */
	static void selecao(Jogador []player, int n, RecordAlgoritmo record){
		int i, j, menor;
...
/*
			System.out.printf("\n");
			for(int k = 0; k < n; k++)
				System.out.printf("%s | ",player[k].getNome());
			System.out.printf("\n");
*/
			menor = i;
			for(j = (i+1); j < n; j++){
				record.sumComp(2);
				if(player[j].getNome().compareTo(player[menor].getNome()) < 0)
					menor = j;	
			} // end for
			
			temp.clone(player[menor]);
			player[menor].clone(player[i]);
			player[i].clone(temp);
			record.sumMov(3);
		} // end for
	} // end selecao()
	...
	public static void main(String []arg){
		...
		iPlayer = getPrimeiraEntrada(player);

		record.startCurrentTime();
		selecao(player, iPlayer, record);	
		record.endCurrentTime();
	
		printPlayers(player, iPlayer);	

		record.recordTime("699415_selecao.txt");
	} // end main()
}
