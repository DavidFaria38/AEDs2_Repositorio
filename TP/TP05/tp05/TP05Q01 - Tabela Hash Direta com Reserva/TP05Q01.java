...
class HashDiretoReserva {
	...
	public HashDiretoReserva (int hashLen, int reservaLen){
	...
	public boolean inserir (Jogador elemento, RecordAlgoritmo record){
		...
		if(elemento != null){
			...
			if(tabela[pos] == null){
				...
				tabela[hashLen + reserva] = elemento.clone();
				reserva++;
				resp = true;
			} // end else if
		} //end if
	...
	public boolean pesquisar (Jogador elemento, RecordAlgoritmo record){
		boolean resp = false;
		
		int pos = h(elemento);
		
		if(tabela[pos].getNome().compareTo(elemento.getNome()) == 0){
			resp = true;
			
		} else {
			for(int i = 0; i < reserva; i++){
				if(tabela[hashLen + i].getNome().compareTo(elemento.getNome()) == 0){
					resp = true;
					i = reserva;
				} // end if
			} // end for
	...
	public boolean pesquisar_Nome (String player, RecordAlgoritmo record){
		...
		for(int pos = 0; pos < tabMaxLen; pos++){
	...
	public Jogador remover (Jogador elemento, RecordAlgoritmo record){
		Jogador tmp = null;
		int pos = h(elemento);
		...
		if(tabela[pos].getNome().compareTo(elemento.getNome()) == 0){
			...
			for(int i = 0; i < reserva; i++){
				if(tabela[hashLen + i].getNome().compareTo(elemento.getNome()) == 0){
					tmp = tabela[hashLen + i];
					tabela[hashLen + i] = null;
					i = reserva;
				} // end if
				record.sumCmp(1); // comparacao if
			} // end for
		} // end else
		record.sumCmp(1); // comparacao if
...
public class TP05Q01 {
	 ...
	 */
	private static void getPrimeiraEntrada(HashDiretoReserva hash, RecordAlgoritmo record) {
		int id = 0;
		String input = "";
	 ...
	 */
	private static void getSegundaEntrada(HashDiretoReserva hash, RecordAlgoritmo record) {
		String input = "";
		// int idInput = 0;
	...
	public static void main(String[] arg) {
		RecordAlgoritmo record = new RecordAlgoritmo("699415_hashReserva.txt");
